	...
	actions: {
		...
		interceptorResponse (context) {
			axios.interceptors.response.use(function (response) {
				...
				if (error.response && error.response.status === 401) {
					if (error.response.data && (error.response.data.messages === 'invalid token' || error.response.data.messages === 'Token Expired')) {
						localStorage.removeItem('accessToken')
						localStorage.removeItem('refreshToken')
						localStorage.removeItem('user')
						// Không redirect ở đây vì có thể gây loop
					}
				}
				return Promise.reject(error)
			})
		...
		getAllHistory (context) {
			return new Promise((resolve, reject) => {
				axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}history`, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } })
					.then((res) => {
		...
		getAllManageOrder (context) {
			return new Promise((resolve, reject) => {
				axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}order/all`)
					.then((res) => {
						console.log(res.data.data)
		...
		deleteHistory (context, { deleteProducts }) {
			return new Promise((resolve, reject) => {
				...
				axios.delete(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}history/delete-history`,
		...
		getCustProfile (context) {
			return new Promise((resolve, reject) => {
				axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}users/profile`, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } })
					.then((res) => {
		...
		getManageOrderId (context) {
			return new Promise((resolve, reject) => {
				axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}order/detail-order/${localStorage.getItem('allorderid')}`)
					.then((res) => {
						console.log(res.data.data)
		...
		updateProfile ({ context, payload }, { email, username, firstName, lastName, address, phone, gender, bod }) {
			return new Promise((resolve, reject) => {
				axios.patch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}users/profile`, {
					email: email,
					username: username,
		...
		markAsDone ({ context, payload }, { orderId }) {
			console.log(orderId)
			return new Promise((resolve, reject) => {
				axios.patch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}order/done`, { order_id: orderId }, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } })
					.then((res) => {
		...
		updateImage (context, file) {
			return new Promise((resolve, reject) => {
				console.log('kebaca?')
				axios.patch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}users/profile`, file, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } })
					.then((res) => {
						Swal.fire(
							'Success!',
		...
		deleteImage (context, payload) {
			return new Promise((resolve, reject) => {
				axios.delete(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/'}users/profile`, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } })
					.then((res) => {
						resolve(res.data)
					})
					.catch((err) => {
